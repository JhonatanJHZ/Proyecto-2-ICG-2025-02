        ...
        if (tipo == "v") {
            ...
            if (polygonVertices.size() >= 3) {
                for (size_t i = 1; i < polygonVertices.size() - 1; ++i) {
                    FaceElement face;
                    // Fix C4244 using static_cast<int>
...
void C3DFigure::saveObject(string filename, vec3 globalPos, quat globalRot, vec3 scale) {
    ...
    int vertexOffset = 1; // OBJ indices son 1-based
    ...
    for (const auto& mesh : subMeshes) {
        ...
        map<int, int> indexMap; // oldGlobalIndex -> newFileIndex (para esta subMesh)

        for (const auto& face : mesh.faces) {
            // Primer paso: asegurar que los vertices de esta cara esten escritos
            for (int i = 0; i < 3; ++i) {
                int oldIdx = face.vertexIndices[i];
                
                if (indexMap.find(oldIdx) == indexMap.end()) {
                    // Calcular nuevo vertice
                    vec3 v = vertices[oldIdx];
                    
                    // Transformacion:
                    // 1. Escala (component-wise)
                    // 2. Rotacion
                    // 3. Traslacion Global
            ...
            // Segundo paso: escribir la cara usando los nuevos indices
            objFile << "f";
            for (int i = 0; i < 3; ++i) {
                int oldIdx = face.vertexIndices[i];
